...
abstract class BaseModel
{
    ...
    {
        //? Step 1: Compute how many records (records count) will be in the             result set (records returned by the server).
        $records_count = $this->count($sql, $args);

        //? Step 2: Instantiate pagination helper class
        ...
        //? Step 3: Get the offset value from PaginationHelper.php
        $offset = $pagination_helper->getOffset();

        //? Step 4: Add the following to the query.
        // ! add space after the " for query
        $sql .= " LIMIT $this->records_per_page OFFSET $offset ";

        //? Step 5: Execute this constrained query.
        $data = (array) $this->run($sql, $args)->fetchAll($fetchMode);

        //? Step 6: Get pagination information (meta).
